// Checkout flow: gated by NEXT_PUBLIC_CHECKOUT_MODE.
//   - "whatsapp" (default) → legacy WhatsApp-only modal; no hub call, no form.
//   - "hub" → POSTs to the central checkout hub; redirects to Shopify or shows
//     a WhatsApp fallback URL when stores are unavailable. Loading overlay is
//     held open for a minimum of 3s so the user perceives the "preparing secure
//     checkout" step even on fast responses.
"use client";
...
export default function OrderSummaryModal(props: OrderSummaryModalProps) {
  if (CHECKOUT_MODE === "hub") {
    return <HubOrderSummaryModal {...props} />;
  }
  return <WhatsAppOrderSummaryModal {...props} />;
}

// ---------------------------------------------------------------------------
// WhatsApp-only variant (default). Mirrors the legacy premium-iptv-uk shape:
// plan summary, addon toggles, one green button that opens wa.me. No form,
// no API call, no loading state.
// ---------------------------------------------------------------------------
function WhatsAppOrderSummaryModal({
  open,
  onClose,
  planName,
  planPrice,
...
// ---------------------------------------------------------------------------
// Hub-wired variant. Collects email + name, POSTs to the checkout hub, and
// either redirects to Shopify or renders a WhatsApp fallback button.
// ---------------------------------------------------------------------------
function HubOrderSummaryModal({
  open,
  onClose,
  planName,
  planPrice,
